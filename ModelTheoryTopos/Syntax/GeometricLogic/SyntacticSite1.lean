...
instance categoryStruct : CategoryStruct (∫ F) where
  ...
  instance categoryStruct : Category (fmlInCtx m) where
    Hom := fmlMap
    id := idMap
    comp := compMap
  ...
  def cover_from_over (xφ : fmlInCtx m) (σ : Over xφ) : fml m.sig xφ.ctx :=
    ...
    --I think it should be yψ ⊢ infdisj ∃x_i. φ (σ j)
  ...
  lemma fmlInCtx.pullback_comm_sq {xφ yψ zξ : fmlInCtx m}
    (f : xφ⟶  yψ) (g:  zξ ⟶ yψ):
     (pullback_fst f g) ≫ f = (pullback_snd f g) ≫ g := by
     --dsimp[categoryStruct]
     ext
     --apply fmlMap_eq
