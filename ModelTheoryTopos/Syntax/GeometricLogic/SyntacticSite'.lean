...
lemma lift1_liftn_ren_left {n1 n2 n : RenCtx} (f : n1 ⟶ n2) (i: Fin n1) :
  ...
  simp only [lift_ren_liftn_ren, Fin.cast, liftn_ren_left]
...
instance categoryStruct : CategoryStruct (∫ F) where
 ...
 Hilbert.proof φ ψ → Hilbert.proof φ (fml.conj φ ψ) := by
  ...
  -- def pullback_fst' {xφ yψ : fmlInCtx m}  (f: xφ ⟶ yψ) (g: Over yψ):
  --   pullback_fst
  --xφ.formula ⊢ yψ.formula.ren map

  -- lemma proof_infdisj_elim_intro
  -- {xφ yψ : fmlInCtx m} (f: xφ ⟶ yψ)
  -- (overs_yψ: Idx_y → Over yψ)
  -- (overs_xφ: Idx_x → Over xφ)
  -- (h1: yψ.formula ⊢ fml.infdisj Idx_y (fun i => cover_from_over yψ (overs_yψ i)))
  -- (h2: maps i ⊢ fml.infdisj Idx )
  -- (comm_sq: ∀ i_x: Idx_x, (overs_xφ i).hom ≫ f.map = sorry ≫ )
  -- : (xφ.formula ⊢  )

  lemma Hilbert.proof.infdisj_intro' (k : SmallUniverse.El a) :
  ...
  def ConveringFamily.pullback  {xφ yψ : fmlInCtx m}  (f: xφ ⟶ yψ) (cf: CoveringFamily yψ):
   CoveringFamily xφ where
     ...
     covering := by
      ...
      simp only[fml.ren_existsn]
      apply Frobenius_reduction_lemma
      apply Hilbert.proof.existn_elim
      simp only[fml.ren_existsn]
      have inl := @pushout.inl RenCtx _ yψ.ctx xφ.ctx (cf.maps k).left.ctx f.map (cf.maps k).hom.map sorry
      have inr := @pushout.inr RenCtx _ yψ.ctx xφ.ctx (cf.maps k).left.ctx f.map (cf.maps k).hom.map sorry
      have l:= @Limits.pushout.desc RenCtx _ (xφ.ctx + (cf.maps k).left.ctx) yψ.ctx xφ.ctx  (cf.maps k).left.ctx
               f.map (cf.maps k).hom.map sorry R.in10 R.in01 sorry
      /-
      ...
      x ----inl------> x + z_k/~
      -/
      fapply Hilbert.proof.existn_intro
       (sorry ∘ @Limits.pushout.desc RenCtx _ (xφ.ctx + (cf.maps k).left.ctx) yψ.ctx xφ.ctx  (cf.maps k).left.ctx f.map (cf.maps k).hom.map sorry R.in10 R.in01 sorry)
      ...
      have d: f1 ⊢ (fml.infdisj cf.index fm') :=
      --
      ...
      generalize h : fml.infdisj cf.index (fun i ↦ cover_from_over xφ (Over.mk (pullback_fst f (cf.maps i).hom))) = a

      intro k
      let fmlk := cover_from_over xφ (Over.mk (pullback_fst f (cf.maps k).hom))
