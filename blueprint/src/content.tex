...
\begin{theorem}[Stability of covering sieves]
  If a family $\{\rho_k\}\to \vec{y}\mid \psi$ is a covering sieve, then the pullback sieve $f^*\{\rho_k\}$ is a covering sieve on $\vec{x}\mid\phi$.

\end{theorem}

\begin{proof}
  If $\{\rho_k\}\to \vec{y}\mid \psi$ is a covering sieve, then it contains a covering family $\{g_{k'}\}\to \vec{y}\mid \psi$
  For each of the 
\end{proof}
